...
// function renderReadability(book) {
//     const listDiv = document.getElementById("d-readability-list");
//     const dropZone = document.getElementById("d-epub-drop");
//     const epubInput = document.getElementById("epub-input");

//     // 1. If data exists, show list
//     if (book.readability) {
...
//         // Helper to make row
//         const row = (label, obj) => `
//             <div class="rd-item" data-tooltip="${obj.label}">
//                 <span class="rd-label">${label}</span>
//                 <span class="rd-score">${obj.score}</span>
//             </div>`;

//         // Lexile is just a string, others are objects {score, label}
//         let html = `
//             <div class="rd-item" data-tooltip="Calibrated Lexile">
//                 <span class="rd-label">Lexile</span>
//                 <span class="rd-score" style="color:var(--accent);">${
//                     r.lexile
//                 }</span>
//             </div>
//             ${row("ATOS", r.atos)}
//             ${row("Flesch-Kincaid", r.fleschKincaid)}
//             ${row("Coleman-Liau", r.coleman)}
//             ${row("Gunning Fog", r.fog)}
//             ${row("SMOG", r.smog)}
//             ${row("ARI", r.ari)}
//         `;
...
//     // 2. If no data, setup Drag & Drop
//     listDiv.classList.add("hidden");
//     dropZone.classList.remove("hidden");

//     // Reset handlers to prevent stacking if render is called multiple times
//     // (A simple way is cloning, or just re-assigning onclick)
//     dropZone.onclick = () => epubInput.click();

//     epubInput.onchange = (e) => handleEpubUpload(book.id, e.target.files[0]);

//     dropZone.ondragover = (e) => {
//         e.preventDefault();
//         dropZone.classList.add("highlight");
//     };
//     dropZone.ondragleave = () => dropZone.classList.remove("highlight");
//     dropZone.ondrop = (e) => {
//         e.preventDefault();
//         dropZone.classList.remove("highlight");
//         if (e.dataTransfer.files.length)
//             handleEpubUpload(book.id, e.dataTransfer.files[0]);
//     };
// }

// async function handleEpubUpload(bookId, file) {
...
//     const dropZone = document.getElementById("d-epub-drop");
//     dropZone.innerHTML =
//         "<p class='epub-loading'>Analyzing book... this may take a moment.</p>";
...
//     try {
//         const res = await fetch("/calculate-readability", {
//             method: "POST",
//             body: formData,
//         });
...
//         if (data.success) {
//             // Update local object structure to match what renderReadability expects
//             const updatedBook = {
//                 ...(await API.getBookDetails(bookId)), // Re-fetch to get everything fresh
//                 readability: data.readability,
//             };
//             // Re-render section
//             renderReadability(updatedBook);
//         } else {
//             dropZone.innerHTML =
//                 "<p style='color:var(--error)'>Analysis failed.</p>";
//         }
//     } catch (err) {
//         console.error(err);
//         dropZone.innerHTML =
//             "<p style='color:var(--error)'>Error uploading.</p>";
//     }
// }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // REMOVED THE ERROR LINE HERE (g is not defined yet)

    renderDetails(book);
    // renderReadability(book); // This is already called INSIDE renderDetails, removed duplicate
    setupInteractiveControls(bookId, book);
    setupModalsAndToggles(book);
});
...
function renderDetails(book) {
    ...
    // Render authors as individual clickable spans
    const authorsDiv = document.getElementById("d-authors-list");
    if (g.authors && g.authors.length > 0) {
        authorsDiv.innerHTML = g.authors
            .map((author) => {
